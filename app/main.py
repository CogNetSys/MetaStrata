...
# Prompt Templates
GRID_DESCRIPTION = f"You are in a {GRID_SIZE} x {GRID_SIZE} field with periodic boundary conditions with {NUM_ENTITIES} other entities. You are free to move around the field and interact with other entities."

DEFAULT_MESSAGE_GENERATION_PROMPT = """
You are entity{entityId} at position ({x}, {y}). {grid_description} 
You have a summary memory of the situation so far: {memory}. 
You received messages from the surrounding entities: {messages}. 
Based on the above, you send a message to the surrounding entities. Your message will reach entities up to distance {distance} away. What message do you send?
...
You have a summary memory of the situation so far: {memory}. 
You received messages from the surrounding entities: {messages}. 
Based on the above, summarize the situation you and the other entities have been in so far for you to remember.
