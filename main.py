...
async def perform_steps(request: StepRequest):
    agents_raw = [
        await redis.hgetall(f"agent:{i}") for i in range(NUM_AGENTS)
    ]
    agents = [
        {
            "id": int(agent["id"]),
    ...
    for _ in range(request.steps):
        ...
        for agent in agents:
            ...
            message_result = await send_llm_request(prompt)
        ...
        for agent in agents:
            ...
            memory_result = await send_llm_request(prompt)
        ...
        # Movement Generation
        for agent in agents:
            prompt = construct_prompt(MOVEMENT_GENERATION_PROMPT, agent, [])
            movement_result = await send_llm_request(prompt)
